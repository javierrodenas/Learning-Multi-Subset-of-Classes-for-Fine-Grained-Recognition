...
def _cfg(url='', **kwargs):
    return {
        'url': url,
        'num_classes': 251, 'input_size': (3, 224, 224), 'pool_size': None,
...
class CSWinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=251, embed_dim=96, depth=[2, 2, 6, 2],
                 split_size=[3, 5, 7],
                 num_heads=12, mlp_ratio=4., qkv_bias=True, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0., hybrid_backbone=None, norm_layer=nn.LayerNorm, use_chk=False,
                 hydra_heads_class_counts: Optional[List[int]] = None):
        super().__init__()
        ...
        self.hydra_heads_class_counts = hydra_heads_class_counts
        self.hydra = True if hydra_heads_class_counts is not None else False
        print("=====================================================")
        print(self.hydra_heads_class_counts)
        print("=====================================================")
        if self.hydra_heads_class_counts is not None:
            total_outputs_heads = sum(self.hydra_heads_class_counts)
            self.fc_out = nn.Linear(total_outputs_heads, 251, bias=False)
            self.fc_out = self.fc_out.cuda()
        #
        # HYDRA CODE
        #
        if self.hydra_heads_class_counts is None:
            ...
            self.hydra_heads = nn.ModuleList([
                ...
                for head_class_count in self.hydra_heads_class_counts])
    ...
    def forward_features(self, x):
        ...
        if not self.hydra:
            ...
            results = []
            for head_idx, hydra_head in enumerate(self.hydra_heads):
                merge, stage, norm, head = hydra_head
...
iccv_heads = []
iccv_heads.append(251)

cluster_df = pd.read_csv('Food251_clusters.csv', dtype='Int64')

if step == 1 or step == 2:
    for cluster in cluster_df:
        iccv_heads.append(cluster_df[cluster].dropna().size + 1)
        print("ADDED clusterr", cluster)
        """
        if cluster == '10':
            break
        """
...
@register_model
def cswin_tiny_hydra(pretrained=False, **kwargs):
    model = CSWinTransformer(patch_size=4, embed_dim=64, depth=[1, 2, 21, 1],
                             split_size=[1, 2, 7, 7], num_heads=[2, 4, 8, 16], mlp_ratio=4.,
                             hydra_heads_class_counts=logmeal_heads, **kwargs)
    model.default_cfg = default_cfgs['cswin_224']
    return model
...
@register_model
def CSWin_144_24322_large_224_hydra_iccv(pretrained=False, **kwargs):
    model = CSWinTransformer(patch_size=4, embed_dim=144, depth=[2, 4, 32, 2],
                             split_size=[1, 2, 7, 7], num_heads=[6, 12, 24, 24], mlp_ratio=4.,
                             hydra_heads_class_counts=iccv_heads, **kwargs)
    model.default_cfg = default_cfgs['cswin_224']
    return model
...
@register_model
def cswin_hydra_base(pretrained=False, **kwargs):
    model = CSWinTransformer(patch_size=4, embed_dim=96, depth=[2, 4, 32, 2],
                             split_size=[1, 2, 12, 12], num_heads=[4, 8, 16, 32], mlp_ratio=4.,
                             hydra_heads_class_counts=[1000, 1000, 1000, 1000, 1000, 1000], **kwargs)
    model.default_cfg = default_cfgs['cswin_384']
    return model

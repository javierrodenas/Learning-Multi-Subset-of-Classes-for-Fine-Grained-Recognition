...
class VOLO(nn.Module):
    ...
    """
    def __init__(self, layers, img_size=224, in_chans=3, num_classes=251, patch_size=8,
                 ...
                 pooling_scale=2, out_kernel=3, out_stride=2, out_padding=1, hydra_heads=[]):
        ...
        if post_layers is not None:
            self.post_network = nn.ModuleList([
                ...
                nn.Linear(embed_dims[-1], hydra_head_classes)
                ])
                for hydra_head_classes in self.hydra_heads])
        ...
        if len(self.hydra_heads) > 1:
            total_outputs_heads = sum(self.hydra_heads)
    ...
    def forward(self, x):
        ...
        if self.post_network is not None:
            results = []
            for post_network_head in self.post_network:
                post_network, norm, linear = post_network_head
                B, N, C = x.shape
                cls_tokens = self.cls_token.expand(B, -1, -1)
                head_x = torch.cat((cls_tokens, x), dim=1)
                for block in post_network:
                    head_x = block(head_x)

                head_x = norm(head_x)
                head_x = linear(head_x.mean(1))
